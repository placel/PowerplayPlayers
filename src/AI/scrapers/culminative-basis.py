...
def get_team_summary_report(headers, season_start_date, date_to_scrape):
    url = f'https://api.nhle.com/stats/rest/en/team/summary?isAggregate=true&isGame=true&sort=%5B%7B%22property%22:%22points%22,%22direction%22:%22DESC%22%7D,%7B%22property%22:%22wins%22,%22direction%22:%22DESC%22%7D,%7B%22property%22:%22teamId%22,%22direction%22:%22ASC%22%7D%5D&start=&limit=50&factCayenneExp=gamesPlayed%3E=1&cayenneExp=gameDate%3C=%22{season_start_date}%2023%3A59%3A59%22%20and%20gameDate%3E=%22{date_to_scrape}%22%20and%20gameTypeId=2'
    req = requests.get(url, headers=headers)
...
def get_team_powerplay_report(headers, season_start_date, date_to_scrape):
...
def get_team_penalty_report(headers, season_start_date, date_to_scrape):
    url = f'https://api.nhle.com/stats/rest/en/team/penalties?isAggregate=true&isGame=true&sort=%5B%7B%22property%22:%22penaltyMinutes%22,%22direction%22:%22DESC%22%7D,%7B%22property%22:%22teamId%22,%22direction%22:%22ASC%22%7D%5D&start=0&limit=50&factCayenneExp=gamesPlayed%3E=1&cayenneExp=gameDate%3C=%22{season_start_date}%2023%3A59%3A59%22%20and%20gameDate%3E=%22{date_to_scrape}%22%20and%20gameTypeId=2'
    req = requests.get(url, headers=headers)
...
def get_team_penalty_kill_report(headers, season_start_date, date_to_scrape):
    url = f'https://api.nhle.com/stats/rest/en/team/penaltykill?isAggregate=true&isGame=true&sort=%5B%7B%22property%22:%22penaltyKillPct%22,%22direction%22:%22DESC%22%7D,%7B%22property%22:%22teamId%22,%22direction%22:%22ASC%22%7D%5D&start=0&limit=50&factCayenneExp=gamesPlayed%3E=1&cayenneExp=gameDate%3C=%22{season_start_date}%2023%3A59%3A59%22%20and%20gameDate%3E=%22{date_to_scrape}%22%20and%20gameTypeId=2'
    req = requests.get(url, headers=headers)
...
def collect_player_reports(headers, seasons_to_scrape):
    for i, season in enumerate(seasons_to_scrape):
        ...
        while date_to_scrape <= END_DATE:
            # Collect the data
            player_summary_report = get_player_summary_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
            player_powerplay_report = get_player_powerplay_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
...
def collect_team_reports(headers, seasons_to_scrape):
    for i, season in enumerate(seasons_to_scrape):
        ...
        while date_to_scrape <= END_DATE:
            # Collect the data
            team_summary_report = get_team_summary_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
            team_powerplay_report = get_team_powerplay_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
            team_penalty_report = get_team_penalty_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
            team_penalty_kill_report = get_team_penalty_kill_report(headers=headers, season_start_date=START_DATE, date_to_scrape=date_to_scrape)
...
def collect_data():
    ...
    SEASONS_TO_SCRAPE = [
        ...
        (datetime.strptime(f'2023-10-10', '%Y-%m-%d').date(), datetime.strptime(f'2024-04-18', '%Y-%m-%d').date())
