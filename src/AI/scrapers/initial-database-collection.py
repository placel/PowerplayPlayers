#########################################################
#
#   THIS SCRIPT WILL COLLECT ALL DATA 
#   FROM THE CURRENT SEASON UP UNTIL TODAYS DATE
#
#########################################################
...
def collect_data():
    ...
    SEASON_START_DATE = datetime.strptime(f'2023-10-10', '%Y-%m-%d').date()
    ...
    SEASONS_TO_SCRAPE = [
        (datetime.strptime(f'{SEASON_START_DATE}', '%Y-%m-%d').date(), datetime.strptime(f'{TODAY_DATE}', '%Y-%m-%d').date())
    ]
